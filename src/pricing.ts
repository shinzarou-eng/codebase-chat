/**
 * Standard list prices ($ per 1M tokens) for the main model lineup —
 * September 2026. Approximations: providers change tiers, intro pricing,
 * peak/off-peak billing and cache discounts often; verify on the provider's
...
export interface ModelPrice {
  ...
  inPerM: number;
  outPerM: number;
  /** Human-readable context window / pricing note. */
  context?: string;
  free?: boolean;
}
...
export const MODEL_PRICES: ModelPrice[] = [
  ...
  { id: 'kimi-k3',           label: 'Kimi K3',             tokMult: 1.05, inPerM: 3,    outPerM: 15,   context: '1M — Moonshot' },
  { id: 'gpt-5.4',           label: 'GPT-5.4',             tokMult: 1.00, inPerM: 2.5,  outPerM: 15,   context: '1.05M' },
  // Mid tier
  { id: 'claude-sonnet-5',   label: 'Claude Sonnet 5',     tokMult: 1.20, inPerM: 2,    outPerM: 10,   context: '1M — $2/$10 made permanent' },
  { id: 'qwen3.8-max',       label: 'Qwen3.8 Max',         tokMult: 1.05, inPerM: 2,    outPerM: 6,    context: '— Alibaba' },
  { id: 'grok-4.6',          label: 'Grok 4.6',            tokMult: 1.05, inPerM: 2,    outPerM: 6,    context: '500k — ×2 above 200k ctx' },
  ...
  { id: 'glm-5.2',           label: 'GLM-5.2',             tokMult: 1.05, inPerM: 1.4,  outPerM: 4.4,  context: '1M — Z.ai' },
  { id: 'muse-spark-1.3',    label: 'Muse Spark 1.3',      tokMult: 1.05, inPerM: 1.25, outPerM: 4.25, context: '1.05M — Meta, contributor ~$0.10/$0.20' },
  // Budget
  { id: 'gemini-3.8-flash',  label: 'Gemini 3.8 Flash',    tokMult: 0.95, inPerM: 0.75, outPerM: 3.75, context: '1M — intro $0.75/$3.75 until Dec 2026' },
  { id: 'gpt-5.4-mini',      label: 'GPT-5.4-mini',        tokMult: 1.00, inPerM: 0.75, outPerM: 4.5,  context: '—' },
  { id: 'deepseek-reasoner', label: 'DeepSeek V4 Pro',     tokMult: 1.00, inPerM: 0.66, outPerM: 1.98, context: '1M — off-peak, ×2 at peak hours' },
  { id: 'claude-haiku-4.5',  label: 'Claude Haiku 4.5',    tokMult: 1.20, inPerM: 0.5,  outPerM: 2.5,  context: '—' },
  { id: 'deepseek-chat',     label: 'DeepSeek V4.1 Flash', tokMult: 1.00, inPerM: 0.22, outPerM: 0.66, context: '1M — off-peak, ×2 at peak hours' },
  { id: 'gpt-5.6-luna',      label: 'GPT-5.6 Luna',        tokMult: 1.00, inPerM: 0.2,  outPerM: 1.2,  context: '—' },
  { id: 'mistral-nemo',      label: 'Mistral Nemo',        tokMult: 1.05, inPerM: 0.02, outPerM: 0.04, context: '128k — ultra budget' },
  // Self-hosted
...
const KEYWORDS: [string, string][] = [
  ...
  ['muse', 'muse-spark-1.3'], ['spark', 'muse-spark-1.3'],
  ['gemini-3.1', 'gemini-3.1-pro'], ['gemini', 'gemini-3.8-flash'],
  ['grok', 'grok-4.6'], ['kimi', 'kimi-k3'], ['qwen', 'qwen3.8-max'], ['glm', 'glm-5.2'],
  ['mistral', 'mistral-nemo'], ['nemo', 'mistral-nemo'],
  ['ollama', 'local'], ['local', 'local'],
  ['gpt-5.6-sol', 'gpt-5.6-sol'], ['gpt-5.6-terra', 'gpt-5.6-terra'], ['gpt-5.6-luna', 'gpt-5.6-luna'],
  ['gpt-5.6', 'gpt-5.6-terra'], ['gpt-5.4-mini', 'gpt-5.4-mini'], ['gpt-5.4', 'gpt-5.4'], ['gpt', 'gpt-5.4'],
];
