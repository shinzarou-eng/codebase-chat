...
// Deterministic engines + context builder shared with the CLI/MCP server.
let buildContext, runCheck, formatCheckMd, runDoctor, formatDoctorMd;
let buildDeterministicReport, addIgnore, removeIgnore, readIgnores, appendHistory;
...
try {
  ({ analyzeProject, formatHealthReport, getChangedFiles, analyzeImpact, formatImpactReport,
    buildContext, runCheck, formatCheckMd, runDoctor, formatDoctorMd,
    buildDeterministicReport, addIgnore, removeIgnore, readIgnores, appendHistory,
  ...
  buildContext = undefined; runCheck = undefined; formatCheckMd = undefined;
  runDoctor = undefined; formatDoctorMd = undefined; buildDeterministicReport = undefined;
  addIgnore = undefined; removeIgnore = undefined; readIgnores = undefined; appendHistory = undefined;
...
function resolveAlias(projectPath) {
  ...
  // Project aliases via env: DSH_PROJECT_ALIASES="name=C:\\Path;other=D:\\Path"
  const aliases = {};
...
async function collectCodebaseContext(projectPath, options = {}) {
  ...
  // One engine for every client: when dist is built, use the symbol-chunk
  // retriever + real token budget shared with the CLI and MCP server. The
  // byte-based walker below stays as the unbuilt-checkout fallback.
  if (buildContext) {
    try {
      reportProgress(lang === "en" ? "Indexing project…" : "Indexation du projet…");
      ...
      return { absProject: r.absProject, context: r.context };
    } catch {
      // index engine unavailable — fall through to the legacy walker
    }
  }
  const isEn = lang === "en";
  const t = isEn ? {
...
function formatRawTasksMarkdown(projectName, metrics, constraints, tasks, lang = "fr") {
  ...
  const footer = isEn
    ? `\n---\n*Made with passion by shinzarou-eng — codebase-chat v${VERSION}*\n`
    : `\n---\n*Fait avec passion par shinzarou-eng — codebase-chat v${VERSION}*\n`;
...
// Patches are keyed by line number, not array position: a line present only
// in "after" is an insert, only in "before" a delete, changed a replace.
function computePatch(beforeBlock, afterBlock) {
  ...
  const beforeByLine = new Map(beforeLines.map((l) => [l.line, l.code]));
  const afterByLine = new Map(afterLines.map((l) => [l.line, l.code]));
  const patches = [];
  for (const a of afterLines) {
    if (!beforeByLine.has(a.line)) {
      patches.push({ line: a.line, op: "insert", oldCode: "", newCode: a.code });
    } else if (beforeByLine.get(a.line) !== a.code) {
      patches.push({ line: a.line, op: "replace", oldCode: beforeByLine.get(a.line), newCode: a.code });
    }
  }
  for (const b of beforeLines) {
    if (!afterByLine.has(b.line)) {
      patches.push({ line: b.line, op: "delete", oldCode: b.code, newCode: "" });
    }
  }
  return patches;
}
...
async function applyTaskPatches(projectRoot, tasks, dryRun = false) {
  ...
  // apply changes — bottom-up so splices never shift lines not yet patched
  for (const [relPath, changes] of fileChanges) {
    const lines = fileContents.get(relPath).split(/\r?\n/);
    const applied = [];
    const all = [];
    for (const { task, patches } of changes) for (const p of patches) all.push({ task, patch: p });
    all.sort((x, y) => y.patch.line - x.patch.line);
    for (const { task, patch } of all) {
      const idx = patch.line - 1;
      const op = patch.op ?? "replace";
      if (op === "insert") {
        const at = Math.min(Math.max(idx, 0), lines.length);
        if (!dryRun) lines.splice(at, 0, patch.newCode);
        applied.push({ id: task.id, line: patch.line, op, status: dryRun ? "dry-run" : "applied" });
        continue;
      }
      if (idx < 0 || idx >= lines.length) {
        applied.push({ id: task.id, line: patch.line, op, status: "out-of-range" });
        continue;
      }
      ...
      if (current !== expected && !current.includes(expected) && !expected.includes(current)) {
        applied.push({ id: task.id, line: patch.line, op, status: "mismatch", expected: patch.oldCode, got: lines[idx] });
        continue;
      }
      if (!dryRun) {
        if (op === "delete") lines.splice(idx, 1);
        else lines[idx] = patch.newCode;
      }
      applied.push({ id: task.id, line: patch.line, op, status: dryRun ? "dry-run" : "applied" });
    }
    if (!dryRun) {
...
async function applyFilePatch(projectRoot, filePath, newContent) {
  if (!(await isWithinProject(projectRoot, filePath))) throw new Error("Chemin cible invalide ou hors du projet.");
...
// Path boundary check: true when target is root itself or a descendant.
// win32 path.relative is case-insensitive; different drives yield an
// absolute rel → false.
...
async function isWithinProject(projectRoot, filePath) {
  const root = resolve(projectRoot);
  const resolved = resolve(root, filePath);
  if (!isInside(root, resolved)) return false;
  // Symlink/junction escape: realpath the deepest existing ancestor of the
  // target (the file itself may not exist yet), then re-append the rest.
  ...
  for (;;) {
    try {
      ...
      if (!isInside(realRoot, rest.length ? join(real, ...rest) : real)) return false;
      break;
    } catch {
      const parent = dirname(cursor);
      if (parent === cursor) break;
      rest.unshift(basename(cursor));
      cursor = parent;
    }
  }
  const protectedAll = PROTECTED_PATHS.map((p) => resolve(p).toLowerCase()).concat(configProtectedPaths(root));
  return !protectedAll.some((p) => isInside(p, resolved.toLowerCase()));
}
...
function apply(ctx) {
  ...
  registerTool(
    tools,
    "codebase_check",
    "Verify the changes vs a git ref: blast radius, complexity, new findings and delta vs the committed baseline (.codebase-chat/baseline.json). Deterministic, returns a red/yellow/green verdict directly, no LLM. Use for 'check my changes', 'verify before merge', 'what did I break'.",
    {
      projectPath: { type: "string", description: "Absolute local path to the project folder. If omitted, the project root is auto-detected from the current working directory." },
    ...
    (value) => value.markdown || "No report generated.",
    async (args, exec) => {
      if (!runCheck) throw new Error("Static analysis module not available.");
      const projectPath = await resolveProjectPath(args.projectPath);
      const report = await runCheck(projectPath, { base: String(args.base || "HEAD"), lang: args.lang === "en" ? "en" : "fr" });
      if (appendHistory) {
        const abs = await findProjectRoot(projectPath).catch(() => projectPath);
        await appendHistory(abs, { ts: new Date().toISOString(), base: report.base, head: report.head, verdict: report.verdict, score: report.score, changed: report.changedFiles.length, added: report.diff.added.length, resolved: report.diff.resolved.length }).catch(() => {});
      }
      return { projectName: basename(report.project), lang: args.lang || "fr", markdown: formatCheckMd(report, args.lang === "en" ? "en" : "fr") };
    }
  );
  ...
  registerTool(
    tools,
    "codebase_doctor",
    "Installation & environment diagnostic: node version, index cache, LLM keys, tree-sitter, baseline staleness, MCP client integrations. Deterministic, no LLM. Use for 'doctor', 'diagnose setup', 'why isn't it working'.",
    {
      projectPath: { type: "string", description: "Absolute local path to the project folder. If omitted, the project root is auto-detected from the current working directory." },
      lang: { type: "string", description: "Report language: 'fr' (default) or 'en'." }
    },
    (value) => value.markdown || "No report generated.",
    async (args, exec) => {
      if (!runDoctor) throw new Error("Static analysis module not available.");
      const projectPath = await resolveProjectPath(args.projectPath);
      const report = await runDoctor(projectPath, args.lang === "en" ? "en" : "fr");
      return { projectName: basename(projectPath), lang: args.lang || "fr", markdown: formatDoctorMd(report, args.lang === "en" ? "en" : "fr") };
    }
  );
  ...
  registerTool(
    tools,
    "codebase_deep_audit",
    "Full deterministic audit (~30 analyses): git churn & bus factor, churn × complexity risk, dependency integrity (undeclared imports, dead deps, lockfile drift), per-function complexity, secrets & sensitive files, env-var coverage, config & README hygiene — all cited file:line. Returns findings directly, no LLM needed. Use for 'deep audit', 'full audit', 'board-ready report'.",
    {
      projectPath: { type: "string", description: "Absolute local path to the project folder. If omitted, the project root is auto-detected from the current working directory." },
      lang: { type: "string", description: "Report language: 'fr' (default) or 'en'." }
    },
    (value) => value.markdown || "No report generated.",
    async (args, exec) => {
      if (!buildDeterministicReport) throw new Error("Static analysis module not available.");
      const projectPath = await resolveProjectPath(args.projectPath);
      const markdown = await buildDeterministicReport(projectPath, args.lang === "en" ? "en" : "fr");
      return { projectName: basename(projectPath), lang: args.lang || "fr", markdown };
    }
  );
  ...
  registerTool(
    tools,
    "codebase_ignore",
    "Silence an audit finding with a justification (stored in .codebase-chat/ignores.json — commit it). `id` may be a full finding id or a prefix like `sec:innerHTML:src/x.ts` covering every such finding in that file. Use action=list/remove to manage entries. Deterministic, no LLM.",
    {
      projectPath: { type: "string", description: "Absolute local path to the project folder. If omitted, the project root is auto-detected from the current working directory." },
      id: { type: "string", description: "Finding id or prefix (see codebase_check output). Omit to list." },
      reason: { type: "string", description: "Why this finding is silenced." },
      action: { type: "string", description: "add | remove | list (default: add, or list when id is omitted)." },
      lang: { type: "string", description: "Report language: 'fr' (default) or 'en'." }
    },
    (value) => value.markdown || "No report generated.",
    async (args, exec) => {
      if (!readIgnores) throw new Error("Static analysis module not available.");
      const projectPath = await resolveProjectPath(args.projectPath);
      const abs = await findProjectRoot(projectPath).catch(() => projectPath);
      ...
      if (action === "list" || !id) {
        ...
        markdown = list.length
          ? list.map((i) => `- \`${i.id}\` — ${i.reason} (${i.createdAt.slice(0, 10)})`).join("\n")
          : "No ignores.";
      } else if (action === "remove") {
        const ok = await removeIgnore(abs, id);
        markdown = ok ? `Removed ignore \`${id}\`.` : `No ignore entry \`${id}\`.`;
      } else {
        const reason = String(args.reason ?? "").trim() || "no reason given";
        const entry = await addIgnore(abs, id, reason);
        markdown = entry ? `Ignored \`${id}\` — ${reason} (.codebase-chat/ignores.json, commit it).` : `\`${id}\` is already ignored.`;
      }
      return { projectName: basename(abs), lang: args.lang || "fr", markdown };
    }
  );
