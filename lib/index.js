...
function getWalkRules(projectRoot) {
  ...
  const rules = {
    ...
    globs: strList(cfg.ignoreGlobs).map(globToRegExp)
  };
  ...
  while (queue.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (!isSkippedDir(entry.name) && !rules.dirs.has(entry.name) && !rules.globs.some((g) => g.test(rel))) queue.push(fullPath);
        continue;
      }
      if (entry.isFile() && !rules.files.has(entry.name) && !rules.globs.some((g) => g.test(rel))) files.push(fullPath);
    }
  }
...
async function buildTree(startDir) {
  ...
  while (queue.length > 0 && lines.length < MAX_TREE_LINES) {
    ...
    for (const entry of entries) {
      ...
      const rel = relative(startDir, fullPath).split(sep).join("/");
      if (entry.isDirectory()) {
        if (!isSkippedDir(entry.name) && !rules.dirs.has(entry.name) && !rules.globs.some((g) => g.test(rel))) {
          lines.push(`${rel}/`);
          queue.push(fullPath);
        }
      } else if (!rules.files.has(entry.name) && !rules.globs.some((g) => g.test(rel))) {
        lines.push(rel);
      }
    }
  }
